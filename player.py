...
class Player:
    ...
    def __init__(self, _id: int, player_type: int = 0):
        self.id = _id
    ...
    def __user_choose_card_on_board(self, include_flipped: bool = False) -> Tuple[int, int]:
        """Prompts the user to choose a covered card on their board"""
        row_col: str = ''
        while not row_col:
            row_col = input('Please enter the row and column of the card you wish to choose:\n(Usage: <row> <col>)\n')
    ...
    def __user_swap_card_on_board(self, card_in_hand: Card):
        """Allows the user to swap the card in his hand for one of the cards on the board"""
        row, col = self.__user_choose_card_on_board(include_flipped=True)
        swapped_card = self.board.apply_move(row, col, card_in_hand)
        print(f'You swapped out a {swapped_card.get_value()} from your board for a {card_in_hand.get_value()}')

    def __user_flip_card(self):
    ...
    def __user_turn(self, deck: Deck, last_turn: bool):
        ...
        g_or_d = None
        while not g_or_d:
            g_or_d = input(f'The card on the top of the pile is {deck.peek_top_of_put_pile().get_value()}.\nWould you like to:\n\t(G) grab the card on the top of the pile\n\t(D) draw a card?\n')
        ...
        if g_or_d == Player.DRAW_CARD:
            ...
            if s_or_f == Player.SWAP_CARD:
                self.__user_swap_card_on_board(card_in_hand)
            elif s_or_f == Player.FLIP_CARD:
                self.__user_flip_card()
    ...
    def play_turn(self, deck, last_turn: bool = False):
        if self.player_type == Player.USER_PLAYER:
            self.__user_turn(deck, last_turn)
